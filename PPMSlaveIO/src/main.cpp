...
#define PPM_FRAME_US      22500  // total frame length in µs
...
static void ppm_isr(void* arg) {
  switch (ppm_phase) {
    ...
    case 0:  // separator HIGH just elapsed → drive LOW (channel gap or sync)
    ...
    case 1:  // channel LOW just elapsed → drive HIGH (separator)
      digitalWrite(PPM_PIN, HIGH);
      esp_timer_start_once(ppm_timer, PPM_PULSE_US);
      ppm_phase = 0;
      break;

    case 2:  // sync LOW just elapsed → snapshot channels, start new frame
      frame_count++;
...
void setup() {
  ...
  // Kick off first separator pulse
  digitalWrite(PPM_PIN, HIGH);
  esp_timer_start_once(ppm_timer, PPM_PULSE_US);
  ppm_phase = 0;
